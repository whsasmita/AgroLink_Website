...
export default function usePrivateChat({ token, currentUser, authLoading }) {
  ...
  useEffect(() => {
    currentUserIdRef.current = String(
      currentUser?.uuid || currentUser?.user_id || currentUser?.id || "",
    );
  }, [currentUser]);

  const isMessageForActiveChat = useCallback((msg) => {
    const myId = String(selfIdRef.current || currentUserIdRef.current || "");
    const activeRecipientId = String(recipientIdRef.current || "");
    const senderId = String(msg?.sender_id || msg?.from_id || msg?.from || "");
    const recipientFromPayload = String(
      msg?.recipient_id || msg?.receiver_id || msg?.to_id || msg?.to || "",
    );

    if (!senderId || !activeRecipientId) return false;

    // Pesan yang di-echo kembali dari server ke pengirim sudah kami render lokal,
    // jadi abaikan agar tidak muncul dobel.
    if (myId && senderId === myId) return false;

    // Terima hanya pesan yang memang berasal dari lawan chat aktif.
    if (senderId !== activeRecipientId) return false;

    // Kalau server menyertakan penerima, pastikan pesan memang ditujukan ke user ini.
    if (recipientFromPayload && myId && recipientFromPayload !== myId)
      return false;
  ...
  const scheduleReconnect = useCallback(
    ...
    [clearReconnectTimer, shouldReconnect],
  );
    ...
    try {
      ...
      ws.onmessage = (evt) => {
        ...
        if (msg && typeof msg === "object") {
          // sistem events (diam)
          if (msg.type === "self_id") {
            const nextSelfId = String(msg.content || "-");
          ...
          if (!isMessageForActiveChat(msg)) return;
          ...
          try {
            const myId = String(
              selfIdRef.current || currentUserIdRef.current || "",
            );
            if (myId && sender && sender !== myId)
              pushRecentContact(myId, sender);
          } catch {
            ("");
          }
          return;
