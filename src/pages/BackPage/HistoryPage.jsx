...
import { MdSearch, MdWork, MdHistory } from "react-icons/md";
...
const HistoryPage = () => {
  ...
  // Empty transactions array - replace with actual data from your API
  const transactions = [];

  // Filter transactions based on search term
  const filteredTransactions = transactions.filter((transaction) =>
    transaction.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    transaction.project_title?.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="flex flex-col sm:flex-row bg-white rounded-lg shadow-sm p-4 justify-between items-start sm:items-center mb-4 gap-4">
        <div>
          ...
          <p className="text-main_text text-sm sm:text-base">
            Lihat riwayat transaksi dan aktivitas Anda
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        {filteredTransactions.length === 0 ? (
          <div className="p-8 text-center">
            <div className="mb-4">
              <MdHistory size={64} className="text-gray-300 mx-auto" />
            </div>
                  ...
                  >
                    ...
                    <td className="px-3 sm:px-6 py-3 sm:py-4">
                      <div className="flex justify-center">
                        <span className="bg-main bg-opacity-10 text-main px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
                          {transaction.status}
                        </span>
                      </div>
                    </td>
